...
{    
    ...
    int number_prob = 100/PDR;
    if(number_prob==0) return 0;
    if(rand() > RAND_MAX/number_prob)
        return 0;
    else
        return 1;
}
    ...
    while(1)
    {
        ...
        switch (state)
        {
            ...
            case 0:{
                ...
                // As expected
                if(!drop &&  rcv_pkt.client_no == 1 && rcv_pkt.seq_no == expect1)
                {
                ...
                else if(!drop && rcv_pkt.client_no == 1 && rcv_pkt.seq_no < expect1 && rcv_pkt.seq_no >= 0)
                {
                ...
                // Done
                else if(!drop && rcv_pkt.client_no == 1 && rcv_pkt.seq_no < 0)
                {
                    snd_ack.seq_no = rcv_pkt.seq_no;
                    snd_ack.TYPE = 'a';
                    snd_ack.client_no = 1;
                ...
                {
                    printf("[-] DROP PKT: Seq. No. = %d, Client no. = %d\n",rcv_pkt.seq_no, rcv_pkt.client_no);
                    state=0;
                }
                break;
            }
            ...
            case 1:{
                ...
                // As expected
                if(!drop && rcv_pkt.client_no == 2 && rcv_pkt.seq_no == expect2)
                {
                    snd_ack.seq_no = rcv_pkt.seq_no;
                    snd_ack.TYPE = 'a';
                    snd_ack.client_no = 2;
                ...
                // For retransmission ack
                else if(!drop && rcv_pkt.client_no == 2 && rcv_pkt.seq_no < expect2 && rcv_pkt.seq_no >= 0)
                {
                    printf("Out of order: %d\n", rcv_pkt.seq_no);
                    snd_ack.seq_no = rcv_pkt.seq_no;
                    snd_ack.TYPE = 'a';
                ...
                // Done
                else if(!drop && rcv_pkt.client_no == 2 && rcv_pkt.seq_no < 0)
                {
                    snd_ack.seq_no = rcv_pkt.seq_no;
                    snd_ack.TYPE = 'a';
                    snd_ack.client_no = 2;
